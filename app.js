...
function renderGoals(){
  ...
  $('#goalList').innerHTML=state.goals.map((g,i)=>`<article class="goal-row card" data-id="${g.id}"><div class="goal-copy"><b>${escapeHtml(g.name)}</b><small>${escapeHtml(g.type||'未分类')} · ${g.timeStart&&g.timeEnd?`建议 ${g.timeStart}–${g.timeEnd} · `:''}${g.levels.map(x=>`${escapeHtml(x.label)} ${x.score}分`).join(' / ')}</small></div><span class="goal-points">${max(g)}分</span><span class="move-buttons" aria-label="调整顺序"><button type="button" class="move-goal" data-direction="-1" ${i===0?'disabled':''} aria-label="上移" title="上移">↑</button><button type="button" class="move-goal" data-direction="1" ${i===state.goals.length-1?'disabled':''} aria-label="下移" title="下移">↓</button></span></article>`).join('') || '<p class="tip">还没有项目，添加一个开始吧。</p>';
  document.querySelectorAll('.goal-row').forEach(row=>row.onclick=e=>{if(!e.target.closest('.move-goal'))openGoal(row.dataset.id)});
  document.querySelectorAll('.move-goal').forEach(b=>b.onclick=e=>{e.stopPropagation();const from=state.goals.findIndex(g=>g.id===b.closest('.goal-row').dataset.id),to=from+(+b.dataset.direction);if(from<0||to<0||to>=state.goals.length)return;[state.goals[from],state.goals[to]]=[state.goals[to],state.goals[from]];save();renderAll()});
}
...
function renderReview(){
  ...
  $('#scoreChart').innerHTML=`<svg class="score-line-chart" viewBox="0 0 ${W} ${H}" role="img" aria-label="近 ${days} 天每日得分曲线"><defs><linearGradient id="scoreFill" x1="0" x2="0" y1="0" y2="1"><stop stop-color="#75a0ff" stop-opacity=".55"/><stop offset="1" stop-color="#75a0ff" stop-opacity="0"/></linearGradient></defs><line class="grid-line" x1="${left}" x2="${W-right}" y1="${top}" y2="${top}"/><line class="grid-line" x1="${left}" x2="${W-right}" y1="${(top+bottom)/2}" y2="${(top+bottom)/2}"/><line class="grid-line" x1="${left}" x2="${W-right}" y1="${bottom}" y2="${bottom}"/><text class="axis-label" x="1" y="${top+3}">${cap}</text><text class="axis-label" x="4" y="${bottom}">0</text><polygon class="score-area" points="${areaPoints}"/><polyline class="score-line" points="${linePoints}"/>${data.map((x,i)=>{const p=points[i],label=x.k.slice(5).replace('-','/');return `<g><title>${x.k}：${x.score} 分</title><circle class="score-dot" cx="${p.x}" cy="${p.y}" r="3"/><text class="score-value" x="${p.x}" y="${Math.max(13,p.y-7)}" text-anchor="middle" font-size="${scoreFont}">${x.score}</text><text class="date-label" transform="rotate(-90 ${p.x} 191)" x="${p.x}" y="191" text-anchor="end" font-size="${dateFont}">${label}</text></g>`}).join('')}</svg>`;
