...
function renderGoals(){
  ...
  $('#goalList').innerHTML=state.goals.map((g,i)=>`<article class="goal-row card" draggable="true" data-id="${g.id}"><span class="drag">⠿</span><div class="goal-copy"><b>${escapeHtml(g.name)}</b><small>${escapeHtml(g.type||'未分类')} · ${g.timeStart&&g.timeEnd?`建议 ${g.timeStart}–${g.timeEnd} · `:''}${g.levels.map(x=>`${escapeHtml(x.label)} ${x.score}分`).join(' / ')}</small></div><span class="goal-points">${max(g)}分</span><span class="move-buttons" aria-label="调整顺序"><button type="button" class="move-goal" data-direction="-1" ${i===0?'disabled':''} aria-label="上移" title="上移">↑</button><button type="button" class="move-goal" data-direction="1" ${i===state.goals.length-1?'disabled':''} aria-label="下移" title="下移">↓</button></span><button class="icon-button edit-goal" aria-label="编辑 ${escapeHtml(g.name)}">›</button></article>`).join('') || '<p class="tip">还没有项目，添加一个开始吧。</p>';
  document.querySelectorAll('.edit-goal').forEach((b,i)=>b.onclick=()=>openGoal(state.goals[i].id));
  document.querySelectorAll('.move-goal').forEach(b=>b.onclick=()=>{const from=state.goals.findIndex(g=>g.id===b.closest('.goal-row').dataset.id),to=from+(+b.dataset.direction);if(from<0||to<0||to>=state.goals.length)return;[state.goals[from],state.goals[to]]=[state.goals[to],state.goals[from]];save();renderAll()});
  enableGoalDrag();
}
