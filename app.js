...
function openDailyReview(key){const [year,month,day]=key.split('-').map(Number),date=new Date(year,month-1,day),record=state.records[key]||{};$('#reviewDialogTitle').textContent=fmtDate(date)+' · 当天明细';$('#dailyReviewList').innerHTML=state.goals.map(g=>{const item=record[g.id],score=item?.score||0;return `<div class="daily-review-row"><div><b>${escapeHtml(g.name)}</b><small>${item?escapeHtml(item.label):'未打卡'}</small></div><span class="daily-review-score ${item?'':'missed'}">${score} / ${max(g)}</span></div>`}).join('');$('#reviewDialog').showModal()}
